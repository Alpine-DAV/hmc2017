...
def wrapper(ML_type, k, data_path, verbose=False, use_online=False, use_mpi=False):
    ...
    # ## only read bubbleShock on rank 0
    # if comm.rank == 0:
    ...
    #     # delete shapeAndSize, shearAndSize
    #     # X = np.delete(X, np.s_[9,11], 1)
    # else:
    #     X = np.empty(shape=(96520, 16), dtype=np.float64)
    #     y = np.empty(96520, dtype=np.float64)
    #     discretized_y = np.empty(len(y))

    # X = comm.Bcast([X, MPI.DOUBLE])
    # y = comm.Bcast([y, MPI.DOUBLE])
    # discretized_y = comm.Bcast([discretized_y, MPI.DOUBLE])
